...
alter table public.subtasks
  drop constraint if exists subtasks_assignee_length;
alter table public.subtasks
  add constraint subtasks_assignee_length check (length(assignee) <= 15);
...
begin
  ...
  if exists (
    ...
    where jsonb_typeof(item) <> 'object'
      ...
      or length(coalesce(item ->> 'assignee', '')) > 15
  ) then
  ...
  insert into public.subtasks (
    id, task_id, title, completed, is_today, sort_order, created_at, assignee, due_date
  )
  select
    (item ->> 'id')::uuid,
    requested_task_id,
